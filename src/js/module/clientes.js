...
export async function getClientesDeEspana() {
    const clientes = await getClientesPorPais("Spain");
    const nombresClientes = clientes.map(({ client_name }) => ({
        client_name,
    }));
...
export async function getClienteYRepresentanteDeVentas() {
    ...
    const clientesInfo = clientes.map(
        ({ client_name, code_employee_sales_manager }) => ({
            client_name,
    ...
    const data = clientesInfo.map(
        async ({ client_name, code_employee_sales_manager }) => {
            const representante = await getEmpleadoPorId(
                code_employee_sales_manager,
            );
...
export async function getClientesConPagoYRepresentanteDeVentas() {
    ...
    const codesClient = pagos.map(({ code_client, payment }) => ({
        code_client,
        payment,
    }));
    ...
    const data = codesClient.map(async ({ code_client, payment }) => {
        ...
        const representante = await getEmpleadoPorId(
            cliente[0].code_employee_sales_manager,
        );
...
export async function getClientesSinPago() {
    ...
    const codeClientesFromPayments = pagos.map(
        ({ code_client }) => code_client,
    );
    const clientesSinPago = clientes.filter(
        ({ client_code }) => !codeClientesFromPayments.includes(client_code),
    );
...
export async function getClientesSinPagoYRepresentanteDeVentasYCiudad() {
    ...
    const data = clientesSinPago.map(
        async ({ client_name, code_employee_sales_manager }) => {
            const representante = await getEmpleadoPorId(
                code_employee_sales_manager,
            );
...
export const getClientsEmploy = async () => {
    ...
    for (let i = 0; i < clients.length; i++) {
        ...
        let [employ] = await getEmployByCode(
            clientsUpdate.code_employee_sales_manager,
        );
        let {
            extension,
            email,
        ...
        let { code_employee_sales_manager, code_office, ...dataUpdate } = data;
